...
# memory does NOT overcommit safely → limits.memory stays under 3/4 host.
...
apps_quota_defaults() {
  ...
  # Memory: requests = 1/4, limits = 3/4 of host. Floors at 1Gi / 3Gi.
  local req_mem_gi lim_mem_gi
  req_mem_gi=$(_max "$(( mem_gb / 4 ))" 1)
  lim_mem_gi=$(_max "$(( mem_gb * 3 / 4 ))" 3)
